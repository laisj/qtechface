...
//图片的中值去噪
IplImage denoising(IplImage* faceImage){
	Mat src , dest;
	src = Mat(faceImage,true);
	denosImage =  cvCreateImage(cvGetSize(faceImage),faceImage->depth,faceImage->nChannels);
	
	medianBlur(src,dest,3);
	*denosImage = IplImage(dest);
	cvShowImage("去噪", denosImage);
	return *denosImage;
}